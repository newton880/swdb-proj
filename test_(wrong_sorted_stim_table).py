...
for path in list_of_paths:
    ...
    # create an empty data frame with columns for each ROI and index (rows)
    sweep_response = pd.DataFrame(index=stimulus_table.index.values,
    ...
    # (restricted to stimulus presentation)
    ...
    for index, row in sorted_table.iterrows():
        ...
        for nc in range(number_cells):
            ...
            # dF/F measured using intersweep as baseline
            sweep_response[str(nc)][index] = 100*((temp/np.mean(temp[:intersweep]))-1)
    ...
    # find visually "responsive" neurons: those whose response was significantly different
    # from blank to at least one condition (see Rust & DiCarlo, 2012) according to KStest
    ...
    # numpy array to store all cell responses for each condition
    conditions = np.ndarray(shape=(len(orivals),len(tfvals)), dtype=list, order='C')
...
#    print "found lifetime sparsenesses"
    ...
    lift_spar_list.append(sig_life_sparseness)
...
names_dict = dict(lift_spar_list=lift_spar_list, pop_spar_list=pop_spar_list, meta_list=meta_list)
